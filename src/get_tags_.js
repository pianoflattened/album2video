...
module.exports = function getTags(form, progressBar) {
	...
	progressBar.makeIndeterminate().then(function() {
		return progressBar.setLabel('collecting files - validating album path..');
	}).then(function() {
		return fs.stat(form.albumDirectory);
	}).then(function(stats) { // check if form.albumDirectory exists + is a directory
		if (stats.isFile()) {
			form.albumDirectory = path.dirname(form.albumDirectory);
		}
		if (stats.isFile() || stats.isDirectory()) {
			return progressBar.setLabel('collecting files - reading ' + form.albumDirectory + '..');
		} else {
			return Promise.reject(new TypeError(form.albumDirectory + 'is not a file or directory'));
		}
	}).then(function() {
		return fs.opendir(form.albumDirectory);
	}).then(function(albumDir) {
		var audioFiles = [];
		let imageFiles = {};
		let discTracks = {};
		let disc, track, fileMimetype, metadata;
		let promises = [];
		
		for (const f of albumDir) { // loop through form.albumDirectory
			if (f.name != "concat.wav") {
				fullpath = path.join(form.albumDirectory, f.name);
				let p = progressBar.setLabel('collecting files - reading ' + fullpath + '..').then(function() {
		...
		return progressBar.error(err.toString());
	});
